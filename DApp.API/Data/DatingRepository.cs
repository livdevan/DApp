...
namespace DApp.API.Data
{
    public class DatingRepository : IDatingRepository
    {
        ...
        public async Task<bool> SaveAll() => await this.context.SaveChangesAsync() > 0;
        ...
        {
            var messages = this.context.Messages.Include(u => u.Sender).ThenInclude(p => p.Photos).Include(u => u.Recipient).ThenInclude(p => p.Photos).AsQueryable();
        ...
        {
            var messages = await this.context.Messages.Include(u => u.Sender).ThenInclude(p => p.Photos).Include(u => u.Recipient).ThenInclude(p => p.Photos)
